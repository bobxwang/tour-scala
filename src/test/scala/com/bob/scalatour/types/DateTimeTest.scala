...
class DateTimeTest extends FunSuite {
  ...
  test("local date") {
    val d1 = LocalDate.of(2015, 1, 1)
    val d2 = LocalDate.of(2015, Month.JANUARY, 1)
    val d3 = LocalDate.ofYearDay(2015, 1)
    assert(d1 == d2)
    assert(d1 == d3)
    assert(d2 == d3)
    ...
    assert(d1.plusYears(1).getYear == 2016)
    assert(d1.minusYears(1).getYear == 2014)
    ...
    assert(d1 == LocalDate.parse("2015-01-01"))
  }
